"""Transfer-learning training script for ResNet50.

This file was created so code from `train_resnet50_transfer_learning.ipynb`
...
def build_transfer_learning_resnet50(num_classes: int):
    try:
...
def main():
    ...
    model = build_transfer_learning_resnet50(num_classes=len(train_dataset.classes))
    ...
    confusion_matrix_path = results_root / "resnet50_transfer_confusion_matrix.png"
    checkpoint_path = results_root / "resnet50_transfer.pth"

    results_summary.to_csv(summary_path, index=False)
    history_df.to_csv(history_path, index=False)
    torch.save(model.state_dict(), checkpoint_path)

    save_training_curves(history, NUM_EPOCHS, figure_path, "ResNet50 Transfer Learning")
    save_confusion_matrix_figure(
        y_true,
        y_pred,
        test_dataset.classes,
        confusion_matrix_path,
        "ResNet50 Transfer Learning Confusion Matrix",
    )
